...
% 1D dry frictional contact (Coulomb conditions). A friction limit force
% must be specified for each contact element, and an imposed gap velocity 
% may be specified. Further, no quadrature of contact stress is done 
% (i.e. the area field of the nonlinear elements is ignored in this 
% function, leading to a unity weighting).
% 
% REFERENCES
...
function [Q,U,LAM,CSTATE,Solinfo] = ...
    sim_friction1D(t,q0,u0,MBmodel,varargin)
disp('=================================================================');
...
% Check for 1D contact; determine imposed gap velocity and friction limit force
...
gamimp = cell(1,nc);
limitFrictionForce = cell(1,nc);
for inl = 1:length(nl)
    % Check friction contact type
    if ~strcmpi(nl{inl}.type,'friction')
        error('This function is for friction contact only.');
    end
    % Handle imposed gap
    if isfield(nl{inl},'imposedGap')
        error('This function does not allow an imposed gap.');
    end
    % Handle friction limit force
    if isfield(nl{inl},'friction_limit_force')
        if isa(nl{inl}.friction_limit_force,'function_handle')
            error('Limit friction force must not be function handle.');
        else
            if isscalar(nl{inl}.friction_limit_force)
                if nl{inl}.friction_limit_force<=0
                    error(['Friction limit force must be positive for closed '
                        'friction contacts.']);
                end
                limitFrictionForce{inl} = nl{inl}.friction_limit_force;
            else
                error(['Friction limit force specified per contact element must' ...
                    ' be scalar.']);
            end
        end
    else
        error('A friction limit force must be specified.');
    end
    ...
    if isfield(nl{inl},'imposedGapVelocity')
        if isa(nl{inl}.imposedGapVelocity,'function_handle')
            ...
            if ~(size(gamimp{inl},1)==1 || size(gamimp{inl},1)~=1) || ...
                    size(gamimp{inl},2)~=nt
                error(['Imposed gap velocity function handle must return '...
                    '1D information (1D tangential gap velocity), ' ...
                    'and return an array whose second ' ....
        ...
        gamimp{inl} = spalloc(1,nt,0);
    end
end
gamimp = vertcat(gamimp{:}); % nb x nt matrix ~ 1D gap velocities
limitFrictionForce = vertcat(limitFrictionForce{:}); % nb x 1 vector ~ 1D friction limit forces
...
for k=1:nt
    ...
    %% ESTIMATE ACTIVE SET
    % This function is specifically designed for closed friction contacts.
    % All contacts are considered active all the time.
    ...
    [lam,output,isSliding] = JORproj(G,c,lam,limitFrictionForce,Sopt);
    ...
    % Evaluate gap and boundary velocity
    gam = Kbbdtinv*lam + c;
    ...
    if mod(k,Sopt.dataStorageFrequency) == 0
        ...
        LAM(:,istep) = lam;
        CSTATE(isSliding,istep) = 1;
        % CSTATE(~isSliding,istep) = 0; % covered by allocation
...
function [lam,output,isSliding] = JORproj(G,c,lam,limitFrictionForce,Sopt)
%========================================================================
...
while (~converged && iter <= Sopt.itermax)
    lamp = projC( lam -  epsAL.*(G*lam+c),limitFrictionForce);
...
% Store output
isSliding = abs(lam)>=limitFrictionForce; % add tolerance or condition on gap velocity?
output.iter = iter-1;
...
function lam = projC(arg,fc)
% THIS FORMULATION IS CLOSEST TO 'HB_nonlinear_forces_DLFT' IN
% 'HB_residual_friction1D.m':
isSliding = abs(arg)>=fc;
lam = arg.*double(~isSliding) + fc.*sign(arg).*double(isSliding);
% % THE FOLLOWING ARE MATHEMATICALLY EQUIVALENT ALTERNATIVE FORMULATIONS.
% % THIS MIGHT BE THE MOST COMPACT ONE:
% xT = sign(xT)*min(abs(xT),limitFrictionForce);
% % THIS MIGHT BE CLOSEST TO 'projC' IN 'sim_contact3D.m':
% normxT = sqrt(xT.^2);
% isSliding = normxT >= limitFrictionForce;
% muxN_normxT = limitFrictionForce(isSliding)./normxT(isSliding);
% muxN_normxT(isnan(muxN_normxT)) = 0;
% xT(isSliding) = muxN_normxT .*xT(isSliding);
end