...
% Check if FE matrices are available
tmp = [ ...
    exist(['FEmodel',filesep,'blade.dof'],'file') ...
    exist(['FEmodel',filesep,'blade.mas'],'file') ...
    exist(['FEmodel',filesep,'blade.sti'],'file') ...
    ];
if any(tmp~=2)
    error(['Required FE matrices not found on search path. ' ...
        'Please extract the three files contained in the ' ...
        '`matrices.zip` archive into the FEmodel folder.']);
end
